	...
	case "$lsb_dist" in
		ubuntu|debian|raspbian)
			...
			(
				...
				$sh_c 'apt-fast update -qq'
				$sh_c "DEBIAN_FRONTEND=noninteractive apt-fast install -y -qq $pre_reqs"
			...
			(
				...
				$sh_c "DEBIAN_FRONTEND=noninteractive apt-fast install -y -qq $pkgs"
			)
			echo_docker_as_nonroot
			exit 0
			;;
